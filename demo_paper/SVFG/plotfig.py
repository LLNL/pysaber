...
def plot2D(x,y,psf,filename,crop=1.0,type='linear',lines=None,vmin=None,vmax=None,xlabel=r'Distance ($\mu m)$',ylabel=r'Distance ($\mu m)$',tick='lin',linesty=['r-']):
    plt.figure()
    ...
    if(type is 'linear'):
        ...
        if(lines is not None):
            lines = [lines] if not isinstance(lines,list) else lines
            for line,sty in zip(lines,linesty):
                plt.plot(line[0],line[1],sty,markersize=LINEIND_WIDTH/2,linewidth=LINEIND_WIDTH/2)
    else:
...
def plot1D(xin,psf,filename,legend=None,crop=1.0,type='linear',xlabel=r'Distance ($\mu m)$',ylabel='Density',aspect=1.0,linesty=None,markstyle=None,markevery=1,title='',FWHMs=None): 
    ...
    if(type is 'linear'):
        for i,(x,p) in enumerate(zip(xin,psf)): 
            x_min = int(x.size//2-crop*(x.size//2))
            x_max = int(x.size//2+crop*(x.size//2))+(1 if x.size % 2 != 0 else 0)
            if linesty is not None and markstyle is not None:
            ...
            elif linesty is not None:
                plt.plot(x[x_min:x_max],p[x_min:x_max],linesty[i],linewidth=LINEIND_WIDTH)
            else:
    ...
    if FWHMs is not None:
        for FWHM in FWHMs:
            plt.axvline(x=FWHM/2,linestyle='--')
...
def plot_srcpsf(delta,full_psf,fileprefix,FWHM_x,FWHM_y):
    filename = fileprefix
    ...
    plot1D(y,full_psf[:,full_psf.shape[1]//2],filename+'_vert.png',crop=0.75,aspect=0.85,FWHMs=[-FWHM_y,FWHM_y],title=r'$FWHM_y={:.2f}\mu m$'.format(FWHM_y))
    plot1D(x,full_psf[full_psf.shape[0]//2],filename+'_horz.png',crop=0.75,aspect=0.85,FWHMs=[-FWHM_x,FWHM_x],title=r'$FWHM_x={:.2f}\mu m$'.format(FWHM_x))
    
def plot_detpsf(delta,full_psf,fileprefix,FWHM_1,FWHM_2,weight_1):
    filename = fileprefix
    y_hwid,x_hwid = delta*(full_psf.shape[0]//2),delta*(full_psf.shape[1]//2)
    ...
    plot1D(x,full_psf[full_psf.shape[0]//2,full_psf.shape[1]//2:],filename+'_log_horz.png',type='log',aspect=0.85,FWHMs=[FWHM_1,FWHM_2],title=r'$FWHM_1={:.2f}\mu m,FWHM_2={:.2f}\mu m$'.format(FWHM_1,FWHM_2)+'\n'+r'$weight_1={:.2f},weight_2={:.2f}$'.format(weight_1,1-weight_1))

def plot_scatpsf(delta,full_psf,fileprefix,FWHM):
    filename = fileprefix
    ...
    plot2D(x,y,full_psf,filename+'.png',crop=0.75,tick='lin')
    plot1D(x,full_psf[full_psf.shape[0]//2],filename+'_horz.png',crop=0.75,aspect=0.85,FWHMs=[-FWHM,FWHM],title=r'$FWHM={:.2f}\mu m$'.format(FWHM))
    
def plot_motpsf(delta,full_psf,fileprefix,FWHM_x,FWHM_y):
    filename = fileprefix
    ...
    plot2D(x,y,full_psf,filename+'.png',crop=0.75,tick='lin')
    plot1D(y,full_psf[:,full_psf.shape[1]//2],filename+'_vert.png',crop=0.75,aspect=0.85,FWHMs=[-FWHM_y,FWHM_y],title=r'$FWHM_y={:.2f}nm$'.format(FWHM_y*1000))
    plot1D(x,full_psf[full_psf.shape[0]//2],filename+'_horz.png',crop=0.75,aspect=0.85,FWHMs=[-FWHM_x,FWHM_x],title=r'$FWHM_x={:.2f}nm$'.format(FWHM_x*1000))
