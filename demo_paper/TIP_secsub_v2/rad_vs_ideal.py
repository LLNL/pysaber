...
from pysaber import get_trans_masks

SAVE_FOLDER = './results_ideal'
...
edge_files = ['data/vert_edge_13mm.tif']
bright_files = ['data/vert_bright.tif']
dark_files = ['data/vert_dark.tif']
sod = [13002.40271]
sdd = [71010.86044]
pix_wid = 0.675
...
for i in range(len(edge_files)):
    ...
    ideal_trans,ideal_mask,rad_mask = get_trans_masks(norm_rad,trans_pars=[0,1],pad=[3,3],edge='straight-edge')
    ...
    plot2D(x,y,img,SAVE_FOLDER+'/norm_rad_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]),lines=lines,linesty=linesty)
    ...
    plot2D(x,y,img,SAVE_FOLDER+'/ideal_trans_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]),lines=lines,linesty=linesty)

    img = rad_mask
    x = np.arange(-img.shape[1]//2,img.shape[1]//2,1)*pix_wid
    y = np.arange(img.shape[0]//2,-img.shape[0]//2,-1)*pix_wid
    plot2D(x,y,img,SAVE_FOLDER+'/rad_mask_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]))

    img = ideal_mask
    x = np.arange(-img.shape[1]//2,img.shape[1]//2,1)*pix_wid
    y = np.arange(img.shape[0]//2,-img.shape[0]//2,-1)*pix_wid
    plot2D(x,y,img,SAVE_FOLDER+'/ideal_mask_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]))
    ...
    plot1D(x_list0,y_list0,SAVE_FOLDER+'/rad_lineprof0_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]),aspect=0.9,legend=legend0)
    plot1D(x_list1,y_list1,SAVE_FOLDER+'/rad_lineprof1_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]),aspect=0.9,legend=legend1)
    plot1D(x_list2,y_list2,SAVE_FOLDER+'/rad_lineprof2_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]),aspect=0.9,legend=legend2)
    plot1D(x_list3,y_list3,SAVE_FOLDER+'/rad_lineprof3_sod{:.0f}_sdd{:.0f}.png'.format(sod[i],sdd[i]),aspect=0.9,legend=legend3)
