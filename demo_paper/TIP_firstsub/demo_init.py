...
for index in indices_run:
    ...
    for j in index:
        ...
        trans_dict,params,bounds = ideal_trans_sharp_edge(rad,pad_factor=[3,3]) #trans_dict has ideal transmission function, masks, etc
    ...
    with open('cost'+file_suffix+'.yml','w') as fid:
        yaml.safe_dump({'cost_opt_source':cost_src,'cost_opt_detector':cost_det},fid,default_flow_style=False)
