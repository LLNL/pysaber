...
def plot_artimages(out_folder,delta,art_tem,art_orig,art_wiener,reg_wiener,art_rwls,reg_rwls):
    ...
    for i in range(len(art_wiener)):
        ...
        plot2D(x,y,img,filename+'_reg{}.png'.format(str(reg_wiener[i]).replace('.','_')),vmin=0.7,vmax=1.0,draw_rect=[(delta*img.shape[0]-delta*(img.shape[0]//4),delta*img.shape[1]-delta*(img.shape[1]//4)),(delta*(img.shape[0]//4),delta*(img.shape[1]//4))])
    ...
    for i in range(len(art_rwls)):
        ...
        plot2D(x,y,img,filename+'_reg{}.png'.format(str(reg_rwls[i]).replace('.','_')),vmin=0.7,vmax=1.0,draw_rect=[(delta*img.shape[0]-delta*(img.shape[0]//4),delta*img.shape[1]-delta*(img.shape[1]//4)),(delta*(img.shape[0]//4),delta*(img.shape[1]//4))])
